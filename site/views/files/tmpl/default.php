...
<form action="<?php echo JRoute::_('index.php?option=com_db8download&view=files'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)): ?>
					...
					<td class="center">
	<a class="btn btn-micro <?php echo $class; ?>" href="<?php echo ($canEdit || $canChange) ? JRoute::_('index.php?option=&task=file.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
		<?php if ($item->state == 1): ?>
				...
				<td>
					...
					<?php
						if (!empty($item->filename)):
							$uploadPath = 'administrator' . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_db8download' . DIRECTORY_SEPARATOR . 'db8downloads' .DIRECTORY_SEPARATOR . $item->filename;

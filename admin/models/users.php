 ...
 */
class Db8downloadModelUsers extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		//Filtering file
		$this->setState('filter.file', $app->getUserStateFromRequest($this->context.'.filter.file', 'filter_file', '', 'string'));
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__db8download_users` AS a');

        
		// Join over the users for the checked out user
		$query->select("uc.name AS editor");
		$query->join("LEFT", "#__users AS uc ON uc.id=a.checked_out");
		// Join over the foreign key 'file'
		$query->select('#__db8download_files_2102385.`name` AS files_name_2102385');
		$query->join('LEFT', '#__db8download_files AS #__db8download_files_2102385 ON #__db8download_files_2102385.`id` = a.`file`');
		...
		//Filtering file
		$filter_file = $this->state->get("filter.file");
		if ($filter_file) {
			$query->where("a.`file` = '".$db->escape($filter_file)."'");
		}
    ...
    public function getItems() {
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->file)) {
				$values = explode(',', $oneItem->file);
			...
			$oneItem->file = !empty($textValue) ? implode(', ', $textValue) : $oneItem->file;
